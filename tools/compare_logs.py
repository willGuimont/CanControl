...
A utility script to load and compare two CAN log CSV files (generated by can_monitor.py).
It calculates message periodicity statistics (average period, std dev) and identifies
dropped packets or missing IDs between two captures.

Usage:
    python tools/compare_logs.py log1.csv log2.csv
"""


def load_log(filename):
    """
    Loads a CAN log CSV into memory.

    Args:
        filename (str): Path to the CSV file.

    Returns:
        dict: A dictionary mapping CAN ID (str) to a list of timestamps (floats).
...
def analyze_capture(name, data):
    """
    Analyzes a single capture dataset to compute message statistics.

    Args:
        name (str): display name for this dataset (e.g. filename).
        data (dict): The dictionary returned by load_log.

    Returns:
        dict: A dictionary of statistics keyed by CAN ID.
              { can_id: { 'count': int, 'period_avg': float, 'period_std': float } }
    """
    print(f"\n--- Analysis: {name} ---")
...
def compare(file1, file2):
    """
    Compares two log files and prints a side-by-side comparison of message periods.
