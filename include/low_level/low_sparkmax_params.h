...
namespace CanControl::LowLevel::SparkMax
{
    ...
    // Voltage Compensation Mode (ID 74) -- How voltage compensation should be handled. Represented by the
    // VoltageCompMode enum
    ...
    // Position Conversion Factor (ID 112) -- Factor to convert encoder counts or other position units to the desired
    // position units.
    MCP2515::ERROR set_parameter_position_conversion_factor(const SparkCanDevice& dev, float value);

    // Velocity Conversion Factor (ID 113) -- Factor to convert encoder counts or other velocity units to the desired
    // velocity units.
    ...
    // Analog Position Conversion (ID 119) -- Conversion factor for position from analog sensor. This value is
    // multiplied by the voltage to give an output value
    MCP2515::ERROR set_parameter_analog_position_conversion(const SparkCanDevice& dev, float value);

    // Analog Velocity Conversion (ID 120) -- Conversion factor for velocity from analog sensor. This value is
    // multiplied by the voltage to give an output value
    MCP2515::ERROR set_parameter_analog_velocity_conversion(const SparkCanDevice& dev, float value);

    // Analog Average Depth (ID 121) -- Number of samples to average for velocity data based on analog encoder input.
    // This value can be between 1 and 64
    MCP2515::ERROR set_parameter_analog_average_depth(const SparkCanDevice& dev, uint32_t value);

    // Analog Sensor Mode (ID 122) -- Absolute: In this mode the sensor position is always read as voltage * conversion
    // factor and reads the absolute position of the sensor. In this mode setPosition() does not have an effect.\n 1
    // Relative: In this mode the voltage difference is summed to calculate a relative position.
    MCP2515::ERROR set_parameter_analog_sensor_mode(const SparkCanDevice& dev, AnalogMode value);

    // Analog Inverted (ID 123) -- When inverted, the voltage is calculated as (ADC Full Scale - ADC Reading). This
    // means that for absolute mode, the sensor value is 3.3V - voltage. In relative mode the direction is reversed.
    ...
    // Alternate Encoder Mode - limit switches are disabled and alternate encoder is enabled. Represented by the
    // CompatibilityPort enum
    MCP2515::ERROR set_parameter_compatibility_port_config(const SparkCanDevice& dev, CompatibilityPort value);

    // Alt Encoder Counts Per Rev (ID 128) -- Number of encoder counts in a single revolution, counting every edge on
    // the A and B lines of a quadrature encoder. (Note: This is different than the CPR spec of the encoder which is
    // 'Cycles per revolution'. This value is 4 * CPR.)
    MCP2515::ERROR set_parameter_alt_encoder_counts_per_rev(const SparkCanDevice& dev, uint32_t value);

    // Alt Encoder Average Depth (ID 129) -- Number of samples to average for velocity data based on quadrature encoder
    ...
    // Alt Encoder Sample Delta (ID 130) -- Delta time value for encoder velocity measurement in 500μs increments. The
    // velocity calculation will take delta the current sample, and the sample x * 500μs behind, and divide by this the
    // sample delta time.
    MCP2515::ERROR set_parameter_alt_encoder_sample_delta(const SparkCanDevice& dev, uint32_t value);

    // Alt Encoder Inverted (ID 131) -- Invert the phase of the encoder sensor. This is useful when the motor direction
    // is opposite of the motor direction.
    MCP2515::ERROR set_parameter_alt_encoder_inverted(const SparkCanDevice& dev, bool value);

    // Alt Encoder Position Conversion (ID 132) -- Value multiplied by the native units (rotations) of the encoder for
    // position.
    MCP2515::ERROR set_parameter_alt_encoder_position_conversion(const SparkCanDevice& dev, float value);

    // Alt Encoder Velocity Conversion (ID 133) -- Value multiplied by the native units (rotations) of the encoder for
    // velocity.
    ...
    // Duty Cycle Position Factor (ID 139) -- Value multiplied by the native units (rotations) of the encoder for
    // position.
    MCP2515::ERROR set_parameter_duty_cycle_position_factor(const SparkCanDevice& dev, float value);

    // Duty Cycle Velocity Factor (ID 140) -- Value multiplied by the native units (rotations) of the encoder for
    // velocity.
    MCP2515::ERROR set_parameter_duty_cycle_velocity_factor(const SparkCanDevice& dev, float value);

    // Duty Cycle Inverted (ID 141) -- Invert the phase of the duty cycle sensor. This is useful when the motor
    // direction is opposite of the encoder direction
    ...
    // Duty Cycle Average Depth (ID 143) -- Log base 2 of the number of samples to average for velocity data based on
    // duty cycle encoder input
    MCP2515::ERROR set_parameter_duty_cycle_average_depth(const SparkCanDevice& dev, uint32_t value);

    // Duty Cycle Offset Legacy (ID 145) -- Legacy method of setting duty cycle offset. This parameter sets the offset
    // after inversion is accounted for.
    ...
    // Duty Cycle Sensor Prescaler (ID 153) -- The Duty Cycle Sensor will set its time base to the device clock divided
    // by (value + 1). For Flex, the device clock is 170MHz, and for MAX, the device clock is 72MHz
    ...
    // MAXMotion Max Accel 0 (ID 167) -- Maximum Acceleration MAXMotion will reach in position mode or velocity mode for
    // slot 0
    ...
    // MAXMotion Max Accel 1 (ID 172) -- Maximum Acceleration MAXMotion will reach in position mode or velocity mode for
    // slot 1
    ...
    // MAXMotion Max Accel 2 (ID 177) -- Maximum Acceleration MAXMotion will reach in position mode or velocity mode for
    // slot 2
    ...
    // MAXMotion Max Accel 3 (ID 182) -- Maximum Acceleration MAXMotion will reach in position mode or velocity mode for
    // slot 3
    ...
    // Follower Mode Is Inverted (ID 195) -- Invert the setpoint when following. This is useful if this motor must spin
    // opposite the leader
    MCP2515::ERROR set_parameter_follower_mode_is_inverted(const SparkCanDevice& dev, bool value);

    // Duty Cycle Encoder Start Pulse Us (ID 196) -- Tells this SPARK that the duty cycle sensor will always output a
    // start (high) pulse of the given microseconds
    MCP2515::ERROR set_parameter_duty_cycle_encoder_start_pulse_us(const SparkCanDevice& dev, float value);

    // Duty Cycle Encoder End Pulse Us (ID 197) -- Tells this SPARK that the duty cycle sensor will always output an end
    // (low) pulse of the given microseconds
